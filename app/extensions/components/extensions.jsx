...
export default function Extensions() {
  ...
  const fetchExtensions = debounce(async () => {
    ...
    if (apiResponse.items) {
      ...
      setPagination({
        ...pagination,
        totalPages: Math.ceil(apiResponse.items.length / apiResponse.limit),
      });
    } else {
      toast.show({
        status: "failure",
  ...
  const selectExtensions = (e) => {
    ...
    checked && extensions.items
      ? setSelectedExtensions([...extensions.items.map((file) => file.zipPath)])
  ...
  const manageExtensions = useCallback(
    (id, type) => {
      adminApi({
        method: "post",
        url: `extensions/${id}`,
  ...
  const tableData = (data) => {
    return (
      ...
      >
        ...
        <Table.Cell>{formatDate(data.creationTime)}</Table.Cell>
        <Table.Cell>{formatDate(data.modificationTime)}</Table.Cell>
        <Table.Cell className="flex justify-around gap-4">
          {data.enabled ? (
            <StopCircleIcon
              className="h-6 w-6 cursor-pointer"
              title="Disable this extension"
              onClick={() => manageExtensions(data.id, true)}
            />
          ) : (
            <PlayCircleIcon
              className="h-6 w-6 cursor-pointer"
          ...
          <TrashIcon
            ...
            onClick={() => extensionDelete(data.id)}
          />
        </Table.Cell>
      </Table.Row>
    );
      ...
      <Card className="mb-4">
        <div className="flex gap-4 justify-between">
          ...
          <div className="flex gap-4">
            <Dropdown label="Server Options">
              <Dropdown.Item
                icon={ServerIcon}
                onClick={() => manageServer("restart")}
              >
                Server Restart
              </Dropdown.Item>
              <Dropdown.Item
                icon={ArrowUpRightIcon}
                onClick={() => manageServer("push")}
              >
                Server Push
              </Dropdown.Item>
              <Dropdown.Item
                icon={CloudArrowDownIcon}
                onClick={() => setLogsModalView(true)}
              >
                Server Logs
              </Dropdown.Item>
            </Dropdown>
