...
const FileConvert = (props) => {
  ...
  const handleDownload = () => {
    const fileName = `${currFiles.name.split(".")[0]}.${
      contentTypes[fileType].extension
    }`;
    let finalData,
      headers,
      allData,
    ...
    if (finalData) {
      const blob = new Blob([finalData], {
        type: contentTypes[fileType].contentType,
      });
  ...
  return (
    <div className="flex items-center phone:flex-row flex-col gap-4 phone:gap-0 justify-center w-full">
      <label
        ref={labelRef}
        htmlFor="occ-file"
        className="flex flex-col items-center justify-center w-4/6 h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600"
      >
        <div className="flex flex-col items-center justify-center pt-5 pb-6 w-full">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            {" "}
            OCC will support only limited file types
          </p>
        </div>
        <input
          id="file-converter"
