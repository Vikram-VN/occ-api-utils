...
export default function Import() {
    ...
    (async () => {
      ...
      if (response.items) {
        ...
        toast.show({
          ...
          message:
            response.message ||
            "Something went wrong while fetching import results",
        });
        setImportList({});
      }
    })();
  ...
  const importItemsHandler = useCallback(
    (isSelected, id) => {
      if (isSelected) {
  ...
  useEffect(() => {
    ...
    const importStatus =
      Object.keys(multiImportList).map((key) => multiImportList[key].processId)
        .length > 0 &&
      setInterval(() => {
        Object.keys(multiImportList).forEach(async (key) => {
    ...
    async (processId, id = "") => {
      ...
      if (response.success === true) {
        ...
        id
          ? setMultiImportList({
              ...multiImportList,
              [id]: { ...multiImportList[id], processId: "" },
            })
        ...
        toast.show({
          ...
          message:
            response.message ||
            "Something went wrong while trying to stop import",
        });
      }
    },
  ...
  const importHandler = useCallback(
    async (fileData) => {
      setMultiImportList({
        ...multiImportList,
        [currentItem]: { ...multiImportList[currentItem], downloadLink: "" },
      });
      const response = await adminApi({
        url: `importProcess`,
        method: "post",
      ...
      if (response.processId) {
        ...
        toast.show({
          ...
          message:
            response.message ||
            "Something went wrong while fetching import process status",
        });
      }
    },
  ...
  const bulkImportHandler = useCallback(
    async (fileData) => {
            ...
            <div className="flex gap-4 items-center justify-end">
              {bundleImport?.processId && (
                <StopCircleIcon
                  ...
                  onClick={() => stopProcess(bundleImport?.processId)}
                />
              )}
              {bundleImport?.processId && (
              ...
              item.formats.length > 0 && (
                <Card className="mb-4 gap-2" key={item.typeName}>
                  <div className="flex gap-2 items-center justify-between">
                    <div className="flex gap-4 items-center">
                      <Checkbox
                        ...
                        disabled={
                          item.formats.length > 0 &&
                          !multiImportList[item.id]?.format
                        }
                        onClick={(ev) =>
                          importItemsHandler(ev.target.checked, item.id)
                        }
                      />
                      <h1 className="text-bold text-2xl">{item.typeName}</h1>
                    </div>
                    <div className="flex gap-4 items-center">
                      {multiImportList[item.id]?.processId && (
                        <StopCircleIcon
                          title="Stop import"
                          className="w-8 h-8 cursor-pointer"
                          onClick={() =>
                  ...
                  <div className="grid md:grid-flow-col gap-4">
                    ...
                    >
                      <CloudArrowUpIcon className="w-6 h-6 mr-2 cursor-pointer" />
                      Import
                    </Button>
                  </div>
                </Card>
