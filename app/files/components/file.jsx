...
export default function Files() {
  ...
  const fetchFiles = debounce(async () => {
    ...
    if (apiResponse.items) {
      ...
      setPagination({
        ...pagination,
        totalPages: Math.ceil(apiResponse.totalResults / apiResponse.limit),
      });
    } else {
      toast.show({
        status: "failure",
  ...
  const selectFiles = (e) => {
    ...
    checked && files.items
      ? setSelectedFiles([...files.items.map((file) => file.path)])
      : setSelectedFiles([]);
    ...
    <React.Fragment>
      ...
      <Card className="mb-4">
        ...
        <div className="flex gap-4">
          ...
          <Button
            type="button"
            disabled={!(selectedFiles.length > 1) && !allFilesSelected}
        ...
        <Table hoverable striped>
          <Table.Head>
            ...
            <Table.HeadCell>
              File name{" "}
              <FunnelIcon
                className="w-8 h-8 inline pl-4 cursor-pointer"
                onClick={() => filterResults("sortBy", "name:desc")}
              />
            </Table.HeadCell>
            <Table.HeadCell>
              Size{" "}
              <FunnelIcon
                className="w-8 h-8 inline pl-4 cursor-pointer"
                onClick={() => filterResults("sortBy", "size:desc")}
              />
            </Table.HeadCell>
            <Table.HeadCell>
              Last Modified{" "}
              <FunnelIcon
                className="w-8 h-8 inline pl-4 cursor-pointer"
                onClick={() => filterResults("sortBy", "lastModified:desc")}
              />
            </Table.HeadCell>
