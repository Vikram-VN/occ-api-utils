...
export default function Organizations() {
  ...
  const paginationHandler = (pageNo) => {
    router.push(`/organizations?page=${pageNo}&field=${queryFilter.field}&operator=${queryFilter.operator}&query=${query}`);
  }
  ...
  const filterOrganizations = debounce(async () => {
    ...
    if (response.items) {
      ...
      toast.show({
        status: 'failure',
        message: response.message || 'Something went wrong while fetching organizations results'
      });
      setResponse({});
    }
    ...
    <React.Fragment>
      ...
      <Table>
        ...
        <Table.Body className='divide-y'>
          {(response.items && response.items.length) > 0 ? response.items.map((item, index) => organizationTableData(item, index + 1)) :
