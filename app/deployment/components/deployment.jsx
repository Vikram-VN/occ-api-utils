...
export const DeploymentTab = () => {
  ...
  const [deploymentPaginationResults, setDeploymentPaginationResults] =
    useState({
      limit: 10,
      totalPages: deployments?.items?.length || 1,
      results:
  ...
  const getDeploymentHistory = debounce(
    async ({ target: { value: appName } }) => {
      ...
      if (apiResponse.items) {
        ...
        setDeploymentPaginationResults({
          ...deploymentPaginationResults,
          totalPages: Math.ceil(
            apiResponse.items.length / deploymentPaginationResults.limit,
          ),
          results: apiResponse.items.slice(
            newOffset,
            newOffset + deploymentPaginationResults.limit,
          ),
        });
      } else {
        toast.show({
          ...
          message:
            apiResponse.message ||
            "Something went wrong while fetching deployment results",
        });
  ...
  const deploymentPaginationHandler = (pageNo) => {
    setDeploymentPaginationResults({
      ...deploymentPaginationResults,
      results: deploymentResults.items.slice(
        newOffset,
        newOffset + deploymentPaginationResults.limit,
      ),
    });
  ...
  const deploymentTableData = (data, index) => {
    return (
      ...
      >
        ...
        <Table.Cell>{formatDate(data.creationDate)}</Table.Cell>
        <Table.Cell>{data.warningMessages}</Table.Cell>
      </Table.Row>
    );
  };
    ...
    <React.Fragment>
      ...
      <Table>
        <Table.Head>
          ...
          <Table.HeadCell>Deployment Id</Table.HeadCell>
          <Table.HeadCell>Status</Table.HeadCell>
          <Table.HeadCell>Created Date</Table.HeadCell>
          <Table.HeadCell>Warning Messages</Table.HeadCell>
        </Table.Head>
        <Table.Body className="divide-y">
          {(deploymentPaginationResults.results &&
            deploymentPaginationResults.results.length) > 0 ? (
            deploymentPaginationResults.results.map((item, index) =>
              deploymentTableData(item, index + 1),
            )
          ) : (
            <Table.Row
              className="bg-white dark:border-gray-700 dark:bg-gray-800"
      ...
      <div className="flex items-center justify-center text-center mt-4 h-20">
        {deploymentPaginationResults.totalPages > 1 && (
          <Pagination
