...
const Publish = (props) => {
  ...
  const [publishPaginationResults, setPublishPaginationResults] = useState({
    limit: 10,
    totalPages: 1,
    results: [],
  });
  ...
  const fetchPublishResults = debounce(async () => {
    if (query) {
      ...
      if (apiResponse.items) {
        ...
        setPublishPaginationResults({
          ...publishPaginationResults,
          totalPages: Math.ceil(
            apiResponse.items.length / publishPaginationResults.limit,
          ),
          results: apiResponse.items.slice(
            newOffset,
            newOffset + publishPaginationResults.limit,
          ),
        });
      } else {
        toast.show({
          ...
          message:
            apiResponse.message ||
            "Something went wrong while fetching publish results",
        });
        setPublishResults({});
      }
    }
  ...
  const publishPaginationHandler = (pageNo) => {
    setPublishPaginationResults({
      ...publishPaginationResults,
      results: publishResults.items.slice(
        newOffset,
        newOffset + publishPaginationResults.limit,
      ),
    });
  ...
  const publishTableData = (data, index) => {
    return (
      ...
      >
        ...
        <Table.Cell>{data.publishInitiator}</Table.Cell>
        <Table.Cell>{data.id}</Table.Cell>
        <Table.Cell>{data.publishInitiatorProfileType}</Table.Cell>
        <Table.Cell>{formatDate(data.startTime)}</Table.Cell>
        <Table.Cell>{formatDate(data.endTime)}</Table.Cell>
      </Table.Row>
    );
  };
  return (
    <React.Fragment>
      ...
      <Table>
        <Table.Head>
          ...
          <Table.HeadCell>Work-Set Name</Table.HeadCell>
          <Table.HeadCell>Publisher</Table.HeadCell>
          <Table.HeadCell>Publish Id</Table.HeadCell>
          <Table.HeadCell>Initiated Through</Table.HeadCell>
          <Table.HeadCell>Start Date</Table.HeadCell>
          <Table.HeadCell>End date</Table.HeadCell>
        </Table.Head>
        <Table.Body className="divide-y">
          {(publishPaginationResults.results &&
            publishPaginationResults.results.length) > 0 ? (
            publishPaginationResults.results.map((item, index) =>
              publishTableData(item, index + 1),
            )
          ) : (
            <Table.Row
              className="bg-white dark:border-gray-700 dark:bg-gray-800"
      ...
      <div className="flex items-center justify-center text-center mt-4 h-20">
        {publishPaginationResults.totalPages > 1 && (
          <Pagination
